...
function executeScript(cmd: string, args: string[], targetCwd: string): Promise<{ code: number | null; output: string }> {
...
async function resolveProjectPath(inputPath: string): Promise<{ resolvedPath: string; isTemporary: boolean; tempBaseDir?: string }> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const projectPathArg = (args?.projectPath || args?.targetPath || ".") as string;

  let resolvedPath = "";
  let isTemporary = false;
  let tempBaseDir: string | undefined = undefined;
  ...
  try {
    ...
    if (isTemporary && tempBaseDir) {
      try {
        ...
        if (idx !== -1) tempDirsToCleanup.splice(idx, 1);
      } catch (err: any) {
